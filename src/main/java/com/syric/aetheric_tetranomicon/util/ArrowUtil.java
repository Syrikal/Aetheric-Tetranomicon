...
public class ArrowUtil {
    ...
    public static AbstractArrow replaceArrow(ItemStack bowStack, AbstractArrow arrow, Player player, int drawProgress) {
    ...
    @SubscribeEvent
    public void arrowEvent(ModularLooseProjectilesEvent event) {

        ItemStack bowStack = event.getFiringStack();
        Player player = event.getShooter();
        int drawProgress = event.getDrawProgress();

        event.addProjectileRemapper(abstractArrow -> {addTags(bowStack, abstractArrow, player, drawProgress); return abstractArrow;});
    }
